...
import { searchDestinationsTool, getSeasonalInfoTool } from '../destination.tools';
...
describe('destination tools', () => {
  ...
  const payload = JSON.parse(result);
    ...
    it('returns friendly fallback when an error occurs', async () => {
      (prismaMock.destination.findMany as jest.Mock).mockRejectedValueOnce(new Error('db down'));
      ...
      expect(payload.nextSteps).toContain('Clarify the destination name or trip style');
    });
  });
  ...
  describe('getSeasonalInfoTool', () => {
    it('returns seasonal data for a specific month when available', async () => {
      ...
      const result = await getSeasonalInfoTool.invoke({ destinationName: 'Paris', month: 5 });
      const payload = JSON.parse(result);
    ...
    it('returns year overview when month is not provided', async () => {
      ...
      const result = await getSeasonalInfoTool.invoke({ destinationName: 'Paris' });
      const payload = JSON.parse(result);
    ...
    it('returns message when destination is not found', async () => {
      (prismaMock.destination.findFirst as jest.Mock).mockResolvedValueOnce(null);

      const result = await getSeasonalInfoTool.invoke({ destinationName: 'Unknown' });
      const payload = JSON.parse(result);
    ...
    it('returns friendly fallback on error', async () => {
      (prismaMock.destination.findFirst as jest.Mock).mockRejectedValueOnce(new Error('timeout'));
      ...
      expect(payload.nextSteps).toContain('Clarify the destination or month you have in mind');
    });
  });
});
